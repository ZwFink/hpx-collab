...
echo "This script is used to run distributed HPX (plain MPI + fork join executor) task bench 8 buran nodes, width 384 fixed width, from 2(6) to 2(24) spread pattern, 5 dependeces/task"
date
...
do
	...
	do
		...
		srun -n 8 -c 48 mpi_executor -type spread -radix 5  -width 384 -steps 1000 -kernel compute_bound -iter ${it} --hpx:threads=48
        	echo "done iter =====================================  "
        	date
	done
done
